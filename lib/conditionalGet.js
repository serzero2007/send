...
function isPreconditionFailure (req, fileContext) {
  // if-match
...
function maybeConditionalGetResponse (req, fileContext) {
  // conditional GET support
  if (isConditionalGET(req)) {
    if (isPreconditionFailure(req, fileContext)) {
      return { status: 412 }
    }

    if (isNotModifiedFailure(req, fileContext)) {
      return {
        status: 304
      }
    }
