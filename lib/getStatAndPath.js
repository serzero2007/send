...
async function getStatAndPathIndex(directory, index) {
...
async function getStatAndPath (_path, { extensions, index }) {
  try {
    const stat = await fsPromise.stat(_path)

    if (hasTrailingSlash(_path) && stat.isDirectory()) {
      return getStatAndPathIndex(_path, index)
    }

    return { stat, path: _path }
  } catch (error) {
