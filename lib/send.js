...
const { getPartialResponseRanges } = require('./Range')
/**
...
async function send (req, _path, options) {
  ...
  const result = await getStatAndPath(parsed.path, context.options)
  context.setStatus(result.status)
  context.setPath(result.path)
  context.setStat(result.stat)
  if (context.finished) return context.response
  ...
  getPartialResponseRanges(context, req)
  if (context.finished) return context.response

  if (context.range === undefined) { return respondFullContent(context) }
