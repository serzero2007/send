...
const { isConditionalGET, isPreconditionFailure, isNotModifiedFailure } = require('./conditionalGet')
...
async function sendFile (req, path, stat, options) {
  ...
  if (ranges === null) {
    const stream = fs.createReadStream(path, { start: options.start, end: options.end })
