...
async function sendFile (req, path, stat, options) {
  ...
  if (options.lastModified) {
    headers['Last-Modified'] = fileContext.modified
  }
  if (options.etag) {
    headers.ETag = fileContext.etag
  }

  // set content-type
  if (fileContext.type) {
    headers['Content-Type'] = fileContext.type
  }

  const conditionGetResponse = maybeConditionalGetResponse(req, fileContext)
  ...
  // Range support
  const ranges = getPartialResponseRanges(req, len, options, fileContext)
