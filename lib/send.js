...
async function getStatAndPath (basepath, { extensions }) {
  try {
    const stat = await fsPromise.stat(basepath)
    const path = basepath
    return { stat, path }
  } catch (error) {
    if (error.code === 'ENOENT' && !extname(basepath) && basepath[basepath.length - 1] !== sep) {
